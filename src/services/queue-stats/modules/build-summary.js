...
module.exports = async (ids, app) => {
  ...
  //sequelize for raw queries.
  const sequelize = app.get('sequelizeClient');

  //we are only interested in TODAY'S data
  ...
  const membersRes = await sequelize.query(
    ...
    'FROM (' +
      ...
      'LEFT JOIN st_states ON queue_member.member_device = st_states.st_extension ' +
      'WHERE ' +
      ...
      'SELECT ' +
        ...
        'LEFT JOIN st_states ON aq_allowed_queue_member.aq_member_device = st_states.st_extension ' +
  ...
  for(let i = 0; i < queues.length; i++){
    ...
    if(agentGroups.hasOwnProperty(queue)){
        ...
        for(let i = 0; i < agentGroupsLength; i++){
          if(extCompleteCallerGroups.hasOwnProperty(agentGroups[queue][i].membername)){
            ...
            for(let k = 0; k < callListLength; k++){
              ...
              const inboundHour = moment.utc(callHour).subtract(callTime, 'seconds').format('hA');
              if(hourlyMetrics.hasOwnProperty(inboundHour)){
                hourlyMetrics[inboundHour].inboundVolume++;
                hourlyMetrics[inboundHour].inboundDuration += callTime;
              } else {
                hourlyMetrics[inboundHour] = {
                  inboundVolume: 1,
                  outboundVolume: 0,
      ...
      //EXAMPLE OF COMPLETE CALL EVENT THAT WAS TRANSFERRED INSTEAD OF HUNG UP.
      ...
      //update the calls leader
      //initialize the agent data
      for(let i = 0; i < agentGroupsLength; i++){
        if(agentGroups[queue][i].inboundTotal > leadingCallsCount && agentGroups[queue][i].inboundTotal > 0){
          leadingCallsCount = agentGroups[queue][i].inboundTotal;
          callLeader = agentGroups[queue][i].membername;
        }
      }
      ...
      if(eventGroups.hasOwnProperty('ABANDON')){
        ...
        for(let i = 0; i < totalAbandon; i++){
          ...
          const inboundHour = moment.utc(callHour).subtract(callerTime, 'seconds').format('hA');
          if(hourlyMetrics.hasOwnProperty(inboundHour)){
            hourlyMetrics[inboundHour].abandonedVolume++;
          } else{
            hourlyMetrics[inboundHour] = {
              inboundVolume: 0,
              outboundVolume: 0,
      ...
      if(eventGroups.hasOwnProperty('EXITWITHKEY')){
        ...
        for(let i = 0; i < totalExitKey; i++){
          const callHour = moment.utc(eventGroups.EXITWITHKEY[i].time).format('hA');
          if(hourlyMetrics.hasOwnProperty(callHour)){
            hourlyMetrics[callHour].exitKey++;
          }else{
            hourlyMetrics[callHour] = {
              inboundVolume: 0,
              outboundVolume: 0,
              inboundDuration: 0,
              outboundDuration: 0,
      ...
      if(eventGroups.hasOwnProperty('EXITWITHTIMEOUT')){
        ...
        for(let i = 0; i < totalExitTimeout; i++){
          const callHour = moment.utc(eventGroups.EXITWITHTIMEOUT[i].time).format('hA');
          if(hourlyMetrics.hasOwnProperty(callHour)){
            hourlyMetrics[callHour].exitTimeout++;
          } else{
            hourlyMetrics[callHour] = {
              inboundVolume: 0,
              outboundVolume: 0,
        ...
        if(extOutboundCallGroups.hasOwnProperty(agentGroups[queue][i].extension)){
          ...
          for(let k = 0; k < callListLength; k++){
            ...
            if(currentCall.disposition === 'ANSWERED'){
              ...
              const callHour = moment.utc(currentCall.start).format('hA');
              if(hourlyMetrics.hasOwnProperty(callHour)){
                hourlyMetrics[callHour].outboundVolume++;
                hourlyMetrics[callHour].outboundDuration += +currentCall.duration;
              } else{
                hourlyMetrics[callHour] = {
                  inboundVolume: 0,
                  outboundVolume: 1,
